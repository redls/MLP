...
class Author3ClassificationDataProvider(DataProvider):
    """Data provider for Author Identification Dataset 3 classification task."""
...
#             inputs = inputs.reshape((-1, 120*25))
        ...
        with open('/Users/lauranechita/mlpractical/data/train.csv') as csvfile:
            ...
            for row in readCSV:
                ...
                if (i != 1):
                    ...
                    self.targets[row[0]] = row[2]
                    listOfWords = row[1].split()
                    if (len(listOfWords) > max_length_of_sentence):
                        max_length_of_sentence = len(listOfWords)
                    for w in listOfWords:
                        ws.add(w)
            for w  in ws:
                self.vocabulary[w] = self.vocab_size
                self.vocab_size = self.vocab_size + 1
        max_length_of_sentence = 15
        inputs2 = []
        target2 = []
        l = []
        for key in self.words:
            listOfWords = self.words[key].split()
            #l = []
            k = 0
            for w in listOfWords:
                k += 1
                if (k <= max_length_of_sentence):
                    l.append(self.vocabulary[w])
            while len(l) % max_length_of_sentence != 0:
                l.append(-1)
            #inputs2.append(l))
            if self.targets[key] == 'EAP':
                target2.append(0)
            if self.targets[key] == 'HPL':
                target2.append(1)
            if self.targets[key] == 'MWS':
                target2.append(2)
        #inputs = inputs2
        print('papipapi')
        ...
        print(inputs.shape)
        print('pampam')
        inputs = np.reshape(l, newshape=(-1, max_length_of_sentence))
        target = np.asarray(target2)
        print(target.shape)
        # pass the loaded data to the parent class __init__
        super(Author3ClassificationDataProvider, self).__init__(
            inputs, target, batch_size, max_num_batches, shuffle_order, rng)

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(Author3ClassificationDataProvider, self).next()
        if self.one_hot:
            return inputs_batch, self.to_one_of_k(targets_batch)
        else:
            return inputs_batch, targets_batch
